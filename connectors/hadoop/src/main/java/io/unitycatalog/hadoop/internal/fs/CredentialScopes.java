 ...
 *
 * <p>A table whose reads span locations beyond its own (a shallow clone's AddFiles point into its
 * base table's location) carries one scope per extra vended credential: the location prefix it
 * covers plus a complete set of credential properties for that prefix. {@link #selectConf} overlays
 * the covering scope's properties — longest covering prefix wins — so {@link CredScopedFileSystem}
 * creates the delegate for those paths with that scope's credentials. The selection is
 * prefix-based, not bucket-based, so scopes that share a bucket are supported, and the overlaid
 * properties may target any cloud scheme.
 */
public final class CredentialScopes {
  ...
  public static Configuration selectConf(URI uri, Configuration conf) {
    ...
    for (Map.Entry<String, String> entry : conf.getPropsWithPrefix(propNs).entrySet()) {
