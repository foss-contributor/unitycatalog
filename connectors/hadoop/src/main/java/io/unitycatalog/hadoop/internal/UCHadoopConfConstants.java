...
public class UCHadoopConfConstants {
  ...
  // Multi-location credential scopes (e.g. a shallow clone's base table location). Scope i in
  // [0, count) carries the location prefix it covers plus a complete set of Hadoop credential
  // properties under its prop. namespace; CredScopedFileSystem overlays the covering scope's
  // properties for paths the table's own credential does not cover.
